/* =============================================================================
 *  Astro-Gann — конфигурация (правь здесь, это «пульт» приложения)
 * =============================================================================
...
LUN.OI_EXTREMES = {
  // пороги ΔОИ ПО БАРУ (tradestats oi_close, все ТФ). Настраиваются в меню.
  barThresholds: [5000, 10000, 50000],
  thresholds: {
...
/* Макс-объёмный узел в баре: круг в точке макс-объёмного суб-бара (M5/M15),
 * радиус ∝ объёму. third — доля трети для цвета (крипта), tf[id].thr — порог по
 * объёму узла на каждый ТФ (задаётся в «Порог макс-объёма…»). */
LUN.MAXVOL = { third: 0.34, tf: {}, byIns: {} };
...
// Стрелки физлиц (FUTOI): порог изменения числа физлиц в свече, при котором
// рисуем стрелку. Открытие вверх → зелёная под свечой, вниз → красная над свечой.
// Маркеры «Физ/Юр на свечах». Пороги — СВОИ на каждый таймфрейм (marksByTf[tfId]),
// значения по умолчанию масштабируются по ТФ (M5→×1, M15→×3, H1→×12, D1≈×160…),
// т.к. FUTOI приходит 5-минутными снимками и в старший бар их попадает больше.
// Стрелки (счета) фильтруются по ЧИСЛУ ЛИЦ, кружки бид/аск («ФАС») — по КОНТРАКТАМ.
// weightMin — удельный вес позиции (|контракты|/|лица|), доп. фильтр крупных лотов.
// ringMax — доп. кольцо у юр-кружка, если лиц в сделке меньше (концентрация капитала).
// tf[tfId] = { 8 порогов + weightMin + ringMax } — своя строка настроек на каждый ТФ.
// Пусто = берутся масштабированные дефолты; правится в меню МОЕКС → «Пороги маркеров».
LUN.FUTOI = { threshold: 100, tf: {}, byIns: {}, netSumAll: false };
// AlgoPack (платная подписка MOEX): онлайн-данные физ/юр и пр. через серверный
// прокси api.php?fn=algopack (ключ на сервере, в браузер не попадает). online:true —
// сначала пробуем реальное время; при отсутствии сессии/ключа тихо падаем на
// публичный ОТЛОЖЕННЫЙ фид (T−15). Выставить false — принудительно только отложенный.
LUN.ALGOPACK = { online: true };
...
// Рисование за барами: behind=true — все построения уходят ПОД свечи (zLevel<0),
// false — поверх (как обычно). Переключается галочкой в Настройках.
LUN.DRAW = { behind: false };
// Экспирации опционов: авто-вертикали с датой/временем. Недельные Si — четверг,
// исполнение в вечерний клиринг MOEX (~18:45 МСК). Реальные даты берём с доски
// опционов (LASTDELDATE) для активного базового актива; если доски нет — считаем
// недельные четверги. weekday: 4 = четверг (0=вс). time — час:мин МСК.
LUN.EXPIRY = { weekday: 4, time: '18:45', showWeek: true, showMonth: true, showQuarter: true };
