...
(function () {
  ...
  async function attach(slot) {
    ...
    // REST-обновление последних баров (для опроса и дозагрузки после фона)
    ...
    const refresh = async () => { try { const b = await prov.fetchCandles(symbolObj, tf); if (b && b.length) { push(b[b.length - 1]); if (b.length > 1) push(b[b.length - 2]); } } catch (e) {} };

    if (provId === 'bybit' && prov.tfMap) {              // настоящий WS
      const iv = prov.tfMap[tf.id] || '60', sym = ins.symbol || symbolObj.ticker, topic = 'kline.' + iv + '.' + sym;
      bybitSub(topic, push);
      subs.set(slot.slotId, { kind: 'ws', topic, handler: push, refresh });
      return;
    }
    // опрос (псевдо-реалтайм)
    const stepMs = ({ minute: 60000, hour: 3600000, day: 86400000 }[tf.type] || 3600000) * tf.span;
    const period = Math.max(3000, Math.min(30000, Math.floor(stepMs / 4)));
    const timer = setInterval(refresh, period); refresh();
    subs.set(slot.slotId, { kind: 'poll', timer, refresh });
    setStatus(conn === 'moex' ? 'поток: MOEX псевдо (~15м)' : 'поток: ' + prov.title + ' опрос', '#e0a030');
  }
  function detach(slot) {
    if (!slot) return; const s = subs.get(slot.slotId); if (!s) return;
    if (s.kind === 'ws') bybitUnsub(s.topic, s.handler); else if (s.timer) clearInterval(s.timer);
    subs.delete(slot.slotId);
  }
  function detachAll() { subs.forEach((s) => { if (s.kind === 'ws') bybitUnsub(s.topic, s.handler); else clearInterval(s.timer); }); subs.clear(); }

  function setConnector(name, on, slots) {
    if (!(name in enabled)) return;
    enabled[name] = on;
    (slots || []).forEach((sl) => attach(sl));
    if (!on) { subs.forEach((s, id) => { /* отвяжем слоты этого коннектора при следующем attach */ }); }
    const any = Object.keys(enabled).some((k) => enabled[k]);
    if (!any) setStatus('поток: выкл', '#6b7280');
  }
